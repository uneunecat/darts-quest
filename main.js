console.log("★ main.js is loaded! (v1.4 Perfect Restoration)");
...
// Status Elements
const elEnemyHP=document.getElementById("enemy-hp"); const elEnemyHPValue=document.getElementById("enemy-hp-value"); const elEnemyHPBar=document.getElementById("enemy-hp-bar");
const elPlayerHP=document.getElementById("player-hp"); const elPlayerHPBar=document.getElementById("player-hp-bar"); const elPlayerMP=document.getElementById("player-mp"); const elPlayerMPBar=document.getElementById("player-mp-bar");
// Badges
...
// --- INIT ---
function resizeGame() {
    const scaler = document.getElementById('game-scaler'); if(!scaler) return;
    ...
    scaler.style.position = "absolute"; scaler.style.top = "50%"; scaler.style.left = "50%";
...
function loadGameData() {
    ...
    if(!allSaveData.slot1) allSaveData.slot1 = null; if(!allSaveData.slot2) allSaveData.slot2 = null; if(!allSaveData.slot3) allSaveData.slot3 = null;
}
loadGameData();
...
function initGameSession(s, cont=false) {
    ...
    setTimeout(()=>{ setupStage(s); setTimeout(()=>{ elChapter.style.opacity=0; setTimeout(()=>{ elChapter.style.display="none"; checkOpeningSkill(); }, 1000); }, 2500); }, 1000);
}

// ★Valkyrie Pre-emptive strike (Added for v1.4)
function checkOpeningSkill() {
    if (stage === 3 && floor === 1) { 
        showSkillCutin("光の護封剣", "wind");
        setTimeout(() => {
            enemy.state.guardType = 'player_cut'; enemy.state.guardTurn = 3;
...
function drawCard() {
    ...
    const cardId = player.deck.pop(); player.hand.push(cardId); updateInfo();
}
function playHandCard(index) {
    if(isProcessing || waitingForChest) return;
    const cardId = player.hand[index]; const card = CARD_DB.find(c => c.id === cardId);
    let cost = card.cost;
...
function drawShopCard(packId) {
    const rand = Math.random() * 100; let targetRarity = "N";
