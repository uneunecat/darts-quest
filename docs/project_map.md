...
| **CONST** | `DECK_SIZE=24`, `HAND_SIZE=5` | v2.0で定数変更 |
...
| **RESULT** | `winBattle`, `nextStep` | 撃破時リソース回復ロジック追加 |
| **UI** | `renderDeckEditor`, `triggerFloatText` | 2カラム描画、演出生成 |

## 3. 重要な依存関係
* **Card IDs:** 500番台以降は Vol.2 (Stage 3 Unlock)。
* **Hand Handling:** 手札が溢れる場合 (`HAND_SIZE`) はドロー不可。
